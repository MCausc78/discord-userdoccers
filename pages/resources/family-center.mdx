...
^1^ If the link type is [`LINKED_USER`](#link-type), the `user_id` and `requestor_id` will be the same.
...
| ----- | -------- | -------------------------------------------------------------- |
| 1     | PENDING  | The Family Center link request has been sent, but not accepted |
| 2     | ACTIVE   | The linked user is currently connected to the requestor        |
| 3     | INACTIVE | The link has been disconnected                                 |
| 4     | DECLINED | The link request was rejected                                  |
| 5     | EXPIRED  | The link request has expired                                   |
...
| Value | Name        | Description                                                              |
| ----- | ----------- | ------------------------------------------------------------------------ |
| 1     | LINKED_USER | The current user accepted the request and is the linked user of the link |
| 2     | REQUESTOR   | The current user sent the request and is the requestor of the link       |
...
| top_user_activities?  | array[[user activity](#user-activity-structure) object]   | List of top user activities                                              |
| top_guild_activities? | array[[guild activity](#guild-activity-structure) object] | List of top guild activities                                             |
| total_spend?          | [total spend](#total-spend-structure) object              | Total amount spent                                                       |
| invoices?             | array[[invoice](#invoice-structure) object]               | List of invoices the linked users made                                   |
...
| 2     | GUILD_ADD           | Guild joined within the last 7 days                                                |
| 3     | USER_INTERACTION    | Users messaged within the last 7 days                                              |
| 4     | GUILD_INTERACTION   | Guilds the linked user has sent messages in within the last 7 days                 |
| 5     | USER_CALLED         | Users called within the last 7 days                                                |
| 6     | TOTAL_VOICE_MINUTES | Duration in minutes how many the linked user spent in calls within the last 7 days |
| 7     | PURCHASES           | Purchases made within the last 7 days                                              |
...
| dms_sent   | integer   | The amount of messages the linked users sent privately to other users |
| call_count | integer   | The amount of calls the linked users have made                        |

###### Guild Activity Structure

| Field         | Type      | Description                                 |
| ------------- | --------- | ------------------------------------------- |
| guild_id      | snowflake | The ID of the guild                         |
| messages_sent | integer   | The amount of messages sent by linked users |
...
| Field          | Type                                                  | Description                                                                                        |
| -------------- | ----------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| invoice_items? | array[[invoice item](#invoice-item-structure) object] | ...                                                                                                |
| total          | integer                                               | ...                                                                                                |
| currency       | string                                                | The lower-cased [ISO 4217](https://en.wikipedia.org/wiki/ISO_4217) currency code the invoice is in |
...
| subscription_plan_id? | snowflake | The ID of the subscription plan the SKU was for |
