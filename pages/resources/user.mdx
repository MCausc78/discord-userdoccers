...
class VerifyPhoneResponseBody(TypedDict):
    token: str

class PhoneHTTPClient(discord.http.HTTPClient):
    def remove_phone(
        self,
        password: str,
        ...
        payload = {
            'password': password,
            'change_phone_reason': change_phone_reason.value
        }
        route = discord.http.Route('DELETE', '/users/@me/phone')
        return self.request(route, json=payload)

    def resend_code(self, phone: str) -> Response[None]:
        payload = {'phone': phone}
        route = discord.http.Route('POST', '/phone-verifications/resend')
        return self.request(route, json=payload)
    ...
    def begin_add_phone(
        ...
        route = discord.http.Route('POST', '/users/@me/phone')
        return self.request(route, json=payload)
    ...
    def begin_reverify_phone(
        ...
        route = discord.http.Route('POST', '/users/@me/phone/reverify')
        return self.request(route, json=payload)

    def reverify_phone(self, phone_token: str, password: str, change_phone_reason: ChangePhoneReason) -> Response[None]:
        payload = {
            'phone_token': phone_token,
            'password': password,
            'change_phone_reason': change_phone_reason.value
    ...
    def verify_phone(self, phone: str, code: str) -> Response[VerifyPhoneResponseBody]:
        payload = {
            'phone': phone,
            'code': code
        }
        route = discord.http.Route('POST', '/phone-verifications/verify')
        return self.request(route, json=payload)
```
...
| Value                | Description                                |
| -------------------- | ------------------------------------------ |
| user_action_required | The phone is used for antispam purposes    |
| user_settings_update | The phone was used from user settings      |
| guild_phone_required | The phone was used to participate in guild |
| mfa_phone_update     | The phone is used for MFA via SMS          |
| contact_sync         | The phone was used to sync contacts        |
