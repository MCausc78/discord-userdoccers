...
| blocked_user_warning_dismissed? ^8^            | boolean                                                            | Whether the user has acknowledged the presence of blocked users in the group DM                                                                                                              |
| safety_warnings? ^3^                           | array[[safety warning](#safety-warning-structure) object]          | The safety warnings for the DM channel                                                                                                                                                       |
| application_id?                                | ?snowflake                                                         | The ID of the application associated with the channel                                                                                                                                        |
...
| app_permissions ^6^                            | string                                                             | Computed permissions for the receiving application in the channel, including overwrites                                                                                                      |
| flags?                                         | integer                                                            | The [channel's flags](#channel-flags)                                                                                                                                                        |
| available_tags?                                | array[[forum tag](#forum-tag-object) object]                       | The tags that can be used in a thread-only channel (max 20)                                                                                                                                  |
...
| status? ^7^                                    | ?string                                                            | The status of the voice channel (max 500 characters)                                                                                                                                         |
...
| is_linkable? ^10^                              | boolean                                                            | Whether the current user can link the channel to a lobby                                                                                                                                     |
| is_viewable_and_writeable_by_all_members? ^10^ | boolean                                                            | Whether all guild members can view the channel and send messages in it                                                                                                                       |
| template?                                      | string                                                             | String to autofill into new forum posts                                                                                                                                                      |
| game_id?                                       | ?snowflake                                                         | The ID of the associated game                                                                                                                                                                |
| last_non_message_activity_timestamp?           | ?ISO8601 timestamp                                                 | Timestamp when the channel last time had non-message activity in it                                                                                                                          |
...
^7^ Only included in the [Gateway Guild](/gateway/gateway-events#gateway-guild-object) object.

^8^ Only included in [Gateway events](/gateway/gateway-events#channels) and when fetched from the [List Private Channels](#list-private-channels) endpoint.

^9^ For ephemeral DM channels, the channel ID will always equal the ID of the other recipient.

^10^ Only included when `with_can_link_lobby` is set to `true` when fetched from the [List Guild Channels](#list-guild-channels) endpoint.

^11^ Channels with the same position are sorted by their ID in ascending order.
...
| 19    | MEDIA_THREAD      | A temporary sub-channel within a `GUILD_MEDIA` channel                                                                                                             |
| 20    | GUILD_SPACE       | A space channel within a guild                                                                                                                                     |
| 21    | GUILD_APP         | An application channel within a guild                                                                                                                              |
...
| 1 \<\< 22     | IS_GAME_INVITES_CHANNEL                       | Guild channel is used as a game invites channel                                                                                                            |
| 1 \<\< 23     | HAS_ONLY_SYSTEM_MESSAGES                      | Channel is marked as only having system messages                                                                                                           |
...
<RouteHeader method="POST" url="/users/@me/channels/group-dm-shell">
...
| flags?                              | ?integer                                                            | The [channel's flags](#channel-flags) (only `IS_SPOILER_CHANNEL`, `IS_GAME_INVITES_CHANNEL` can be set)                                                                              | All                                    |
| application_id?                     | snowflake                                                           | The ID of the application associated with the channel                                                                                                                                | Guild App                              |
...
| flags?                              | integer                                                             | The [channel's flags](#channel-flags) (only `GUILD_FEED_REMOVED`, `PINNED`, `ACTIVE_CHANNELS_REMOVED`, `REQUIRE_TAG`, `IS_SPOILER_CHANNEL`, and `IS_GAME_INVITES_CHANNEL` can be set) | All                                                                |
| available_tags? ^3^                 | array[partial [forum tag](#forum-tag-object) object]                | The tags that can be used in a thread-only channel (max 20)                                                                                                                           | Forum, Media                                                       |
...
<RouteHeader method="POST" url="/users/@me/channels/bulk-leave">
