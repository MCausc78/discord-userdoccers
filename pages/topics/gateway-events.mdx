...
Used to create a stream. Fires a [Stream Create](#stream-create) Gateway event.

###### Create Stream Structure

| Field      | Type       | Description                                |
| ---------- | ---------- | ------------------------------------------ |
| type       | string     | The type of the stream (`call` or `guild`) |
| guild_id   | ?snowflake | The ID of the guild                        |
| channel_id | snowflake  | The ID of the channel to start stream in   |

#### Delete Stream

Used to delete a stream. Fires a [Stream Delete](#stream-delete) Gateway event.
...
Used to watch a stream. Fires a [Stream Update](#stream-update) Gateway event.
...
Used to pause/resume a client stream. Fires a [Stream Update](#stream-update) Gateway event.
...
| [Stream Create](#stream-create)                                                   | Channel stream created                                                                                        |
| [Stream Update](#stream-update)                                                   | Channel stream updated                                                                                        |
| [Stream Delete](#stream-delete)                                                   | Channel stream deleted                                                                                        |
| [Stream Server Update](#stream-server-update)                                     | Stream connection server was updated                                                                          |
...
Sent when a stream is created. Inner payload is a [stream](/resources/stream#stream-object) object.

#### Stream Update

Sent when a stream is updated. Inner payload is a [stream](/resources/stream#stream-object) object.
...
| Field      | Type   | Description                   |
| ---------- | ------ | ----------------------------- |
| reason     | string | The stream deletion reason    |
| stream_key | string | The encoded key of the stream |

#### Stream Server Update

Sent when a guild or call's stream server is updated. This is sent when initially creating a stream, and when the current stream instance fails over to a new server.

<Alert type="warn">

A `null` endpoint means that the stream server allocated has gone away and is trying to be reallocated. You should attempt to disconnect from the currently connected stream server, and not attempt to reconnect until a new stream server is allocated.
...
| Field       | Type       | Description                                          |
| ----------- | ---------- | ---------------------------------------------------- |
| token       | string     | The stream connection token                          |
| guild_id    | ?snowflake | The guild this stream server update is for           |
| channel_id? | snowflake  | The private channel this stream server update is for |
| stream_key  | string     | The stream key                                       |
| endpoint    | ?string    | The stream server host                               |

###### Example Stream Server Update Payload

```json
{
  "token": "my_token",
  "guild_id": null,
  "stream_key": "guild:1015060230222131221:1028117287716261939:852892297661906993",
