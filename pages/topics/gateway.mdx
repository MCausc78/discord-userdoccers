...
| 1 << 18 | DEBOUNCE_VOICE_STATE_UPDATES          | Debounces voice state update events, preventing the client from receiving multiple [Voice State Update](/topics/gateway-events#voice-state-update) events for the same guild within a short period of time; clients will receive a single [Voice State Update Batch](/topics/gateway-events#voice-state-update-batch) event instead            |
| 1 << 19 | DEBOUNCE_MESSAGE_POLL_VOTES           | Debounces message poll vote events, preventing the client from receiving multiple [Message Poll Vote Add](/topics/gateway-events#message-poll-vote-add) events for the same message within a short period of time; clients will receive a single [Message Poll Vote Add Many](/topics/gateway-events#message-poll-vote-add-many) event instead |
| 1 << 20 | DEBOUNCE_VOICE_STATE_UPDATES_V2 ^3^   | Debounces voice state update events, preventing the client from receiving multiple [Voice State Update](/topics/gateway-events#voice-state-update) events for the same guild within a short period of time; clients will receive a single [Voice State Update Batch](/topics/gateway-events#voice-state-update-batch) event instead            |
...
^3^ Supersedes `DEBOUNCE_VOICE_STATE_UPDATES`. If enabled, `DEBOUNCE_VOICE_STATE_UPDATES` will be ignored.
