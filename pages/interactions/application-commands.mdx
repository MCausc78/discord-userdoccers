...
| ------------------- | ---- | ------------------------------------------------------------------------------- |
| CHAT_INPUT          | 1    | Slash commands; a text-based command that shows up when an user types `/`       |
...
To make a **global** command, make a HTTP POST call like this:
...
^1^ If `id` field is the application ID instead of a command ID, the permissions apply to all commands that do not contain explicit overwrites.
...
| Field      | Type      | Description                                                                                      |
| ---------- | --------- | ------------------------------------------------------------------------------------------------ |
| id ^1^     | snowflake | The ID of the role, user or channel                                                              |
...
^1^ If `type` is `CHANNEL`, a special value of `guild_id - 1` can be used to represent all channels in the guild.
...
Need to keep some of your commands safe from prying eyes, or only available to the right people? Commands support permission overwrites! For both guild _and_ global commands of all types (except `PRIMARY_ENTRY_POINT`),
you can enable or disable a specific user or role in a guild from using a command.

<Alert type="note">

If you don't have permissions to use a command, they will not show up in the command picker.

</Alert>

You can also set a `default_member_permissions` on your commands if you want them to be disabled by default when your app is added to a new guild.
Setting `default_permission` to `false` will disallow _anyone_ in a guild from using the command—even Administrators and guild owners—unless a specific overwrite is configured. It will also disable the command from being usable in DMs.
...
- Get the guild permissions for an user or a role
- Get the permissions for an user or a role on a specific channel
- Change the guild permissions for an user or a role
- Change the permissions for an user or a role on a specific channel
...
Now, we need some arguments! If we chose `user`, we need to be able to pick an user; if we chose `role`, we need to be able to pick a role. We also want to be able to pick between guild-level permissions and channel-specific permissions. For that, we can use optional arguments:
...
When someone uses an user command, your application will receive an interaction:
...
| name                            | string                                                                            | The name of the command (1-32 characters)                                                                                                                   | All                   |
| name_localizations?             | ?map[string, string]                                                              | The localized name for each [locale](/reference#locales) (values follow same restrictions as `name`)                                                        | All                   |
| description? ^1^                | string                                                                            | The description of the command (1-100 characters)                                                                                                           | All                   |
| description_localizations?      | ?map[string, string]                                                              | The localized description for each [locale](/reference#locales) (values follow same restrictions as `description`)                                          | All                   |
| options?                        | array[[application command option](#application-command-option-structure) object] | The options for the command (max 25)                                                                                                                        | `CHAT_INPUT`          |
| default_member_permissions?     | ?string                                                                           | The default required permissions to call the command                                                                                                        | All                   |
...
^1^ Required for [`CHAT_INPUT` commands](#application-command-type).
...
Returns a list of [application command](#application-command-object) objects registered for the given guild.
...
Create a new guild command. Returns an [application command](#application-command-object) object. If the command did not already exist, it will count toward daily application command create limits. Fires [Guild Application Command Index Update](/topics/gateway-events#guild-application-command-index-update) Gateway event.
...
Deletes a guild application command. Returns a 204 empty response on success. Fires [Guild Application Command Index Update](/topics/gateway-events#guild-application-command-index-update) Gateway event.
